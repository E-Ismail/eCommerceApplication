...
Some examples are as below: To create a new user for example, you would send a POST request to: [http://localhost:8080/api/user/create](http://localhost:8080/api/user/create) with an example body like
...
2.  JWT does not ship as a part of spring security, so you will have to add the
    -   java-jwt dependency to your project.
3.  Spring Boot ships with an automatically configured security module that must be disabled, as we will be implementing our own. This must be done in the Application class.
4.  Create password for the user
5.  Once that is disabled, you will need to implement 4 classes (at minimum, you can break it down however you like):
